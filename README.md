...
System Workflow
The following stages define the end-to-end process from data preparation to clinical inference:

Data Preprocessing: Images are resized to 150x150 pixels and normalized using a 1/255 rescale factor.

Model Training: A Sequential CNN extracts spatial features through multiple convolutional and pooling layers.

Optimization: The model is compiled using the Adam optimizer and Categorical Crossentropy loss function.

Serialization: The trained Keras model is exported to the Open Neural Network Exchange (ONNX) format for optimized inference performance.

Deployment: A Streamlit interface handles image uploads, runs the ONNX session, and visualizes diagnostic confidence scores.
